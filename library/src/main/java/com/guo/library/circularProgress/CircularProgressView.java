package com.guo.library.circularProgress;
...
public class CircularProgressView extends View {
    ...
    private float maxProgress, indeterminateSweep, indeterminateRotateOffset;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isProgress) {
            float width = mPaint.measureText(currentProgress + "%");
            float height = mPaint.ascent() + mPaint.descent();
            canvas.drawText(currentProgress + "%", canvas.getWidth() / 2 - width / 2, canvas.getHeight() / 2 - height / 2, mPaint);
        }
